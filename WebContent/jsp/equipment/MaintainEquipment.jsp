		...
		$("#dialog-maintain-form").dialog({
			...
			buttons : {
						"保養設備" : function() {
							...
							$.ajax({
								url : "./MaintainEquipmentProcess.jsp",
								type : "POST",
								data : {
									"data" : myJson
								},
	...
	function maintainEquipment(id) {
		...
		$.post("./ViewEquipmentProcess.jsp", {
			...
			if (status == 'success') {
				...
				$("#maintain-form").find("#equipmentId").prop("disabled", true);
				$("#maintain-form").find("#manufactoryName").prop("disabled", true);
				$("#maintain-form").find("#constructionType").prop("disabled", true);
			}
		...
		var table = $('#single-maintain-table-1').DataTable( {
	        ...
	        "scrollCollapse": true,		
			"ajax": {  
			    "url": "./ViewEquipmentProcess-tab2.jsp",  
			    "data": {  
			        "id": id  
			    }, 
