...
var view_obj = {		
		...
		initReactComponent : function (action){	
			...
			var iamList = "<%=(String)session.getAttribute("人員管理")%>".split(",");
			var iamObj = checkIAM(iamList);
			view_obj.stateButtons_person = ReactDOM.render(React.createElement(stateButtons, {domId:action,typeId:"person",domIdCN:"人員",canCheck:iamObj.canCheck,canApprove:iamObj.canApprove,canReject:iamObj.canReject}), container);
			...
			action_obj.licenseList_select_Action_subscribe(view_obj.licenseList);
			action_obj.licenseList_deselect_Action_subscribe(view_obj.licenseList);
			action_obj.checkButton_click_Action_subscribe(view_obj.licenseList);
			action_obj.approveButton_click_Action_subscribe(view_obj.licenseList);
			action_obj.rejectButton_click_Action_subscribe(view_obj.licenseList);
			
			container = document.getElementById('stateButtons_license');
			var iamList = "<%=(String)session.getAttribute("操作證")%>".split(",");
			var iamObj = checkIAM(iamList);
			view_obj.stateButtons_license = ReactDOM.render(React.createElement(stateButtons, {domId:action,typeId:"license",domIdCN:"操作證紀錄",canCheck:iamObj.canCheck,canApprove:iamObj.canApprove,canReject:iamObj.canReject}), container);
			...
			action_obj.trainingList_select_Action_subscribe(view_obj.trainingList);
			action_obj.trainingList_deselect_Action_subscribe(view_obj.trainingList);
			action_obj.checkButton_click_Action_subscribe(view_obj.trainingList);
			action_obj.approveButton_click_Action_subscribe(view_obj.trainingList);
			action_obj.rejectButton_click_Action_subscribe(view_obj.trainingList);
			
			container = document.getElementById('stateButtons_training');
			var iamList = "<%=(String)session.getAttribute("訓練紀錄")%>".split(",");
			var iamObj = checkIAM(iamList);
			view_obj.stateButtons_training = ReactDOM.render(React.createElement(stateButtons, {domId:action,typeId:"training",domIdCN:"訓練紀錄",canCheck:iamObj.canCheck,canApprove:iamObj.canApprove,canReject:iamObj.canReject}), container);
			...
			action_obj.eventList_select_Action_subscribe(view_obj.eventList);
			action_obj.eventList_deselect_Action_subscribe(view_obj.eventList);
			action_obj.checkButton_click_Action_subscribe(view_obj.eventList);
			action_obj.approveButton_click_Action_subscribe(view_obj.eventList);
			action_obj.rejectButton_click_Action_subscribe(view_obj.eventList);
			
			container = document.getElementById('stateButtons_event');
			var iamList = "<%=(String)session.getAttribute("特殊事蹟")%>".split(",");
			var iamObj = checkIAM(iamList);
			view_obj.stateButtons_event = ReactDOM.render(React.createElement(stateButtons, {domId:action,typeId:"event",domIdCN:"特殊事蹟紀錄",canCheck:iamObj.canCheck,canApprove:iamObj.canApprove,canReject:iamObj.canReject}), container);
